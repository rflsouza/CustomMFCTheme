...
#include "MFCVisualManagerDeviant.h"	// <-- Added to support deviant themes!
#include "MainFrm.h"
#include "CodeDlg.h"
...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
	...
	ON_COMMAND_RANGE(ID_VIEW_APPLOOK_DEVIANT, ID_VIEW_APPLOOK_DEVIANT, OnAppLook)	// Added to support deviant themes!
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_APPLOOK_2007, ID_VIEW_APPLOOK_2007_3, OnUpdateAppLook)
	ON_UPDATE_COMMAND_UI_RANGE(ID_VIEW_APPLOOK_DEVIANT, ID_VIEW_APPLOOK_DEVIANT, OnUpdateAppLook)	// Added to support deviant themes!
	...
	ON_COMMAND(ID_RIBBON_CBTN_GRADSTART, OnBtnRibbonCategoryGradStart)
	ON_COMMAND(ID_RIBBON_CBTN_GRADEND, OnBtnRibbonCategoryGradEnd)
	ON_COMMAND(ID_RIBBONPANEL_CBTN_GRADSTART, OnBtnRibbonPanelGradStart)
	ON_COMMAND(ID_RIBBONPANEL_CBTN_GRADEND, OnBtnRibbonPanelGradEnd)
	ON_COMMAND(ID_RIBBONPANEL_CBTN_TEXT, OnBtnRibbonPanelCaptionText)
	ON_COMMAND(ID_RIBBONPANEL_CBTN_TEXTHILITE, OnBtnRibbonPanelCaptopnTextHilite)
	ON_COMMAND(ID_RIBBONBARBK_CBTN, OnBtnRibbonBarBackground)
	ON_COMMAND(ID_RIBBONPANEL_CBTN_OUTLINE, OnBtnRibbonPanelOutline)
	ON_COMMAND(ID_RIBBONPANEL_CBTN_BUTTONTEXT, OnBtnRibbonPanelButtonText)
	ON_COMMAND(ID_RIBBONBARTXT_CBTN, OnBtnRibbonBarText)
	ON_COMMAND(ID_RIBBONBARTXTHILITE_CBTN, OnBtnRibbonBarTextHilite)
	ON_COMMAND(ID_RIBBONBARTOOLS_CBTN, OnBtnRibbonBarGetCode)
	ON_COMMAND(IDRIBBONPANELCAPTIONBK_CBTN, OnBtnRibbonPanelCaptionBk)
END_MESSAGE_MAP()
...
CMainFrame::CMainFrame()
	...
	, m_nProgressValue2 (-1)
{
	m_nAppLook = theApp.GetInt (_T("ApplicationLook"), ID_VIEW_APPLOOK_DEVIANT);
}
...
void CMainFrame::CreateDocumentColors ()
{
	...
	nNumColours = sizeof (colorsMain) / sizeof (ColorTableEntry);	// --> ToUnicode check 'sizeof' : sizeof() returns size in bytes, NOT characters!
	...
	nNumColours = sizeof (colorsAdditional) / sizeof (ColorTableEntry);	// --> ToUnicode check 'sizeof' : sizeof() returns size in bytes, NOT characters!
	...
	nNumColours = sizeof (colorsStandard) / sizeof (ColorTableEntry);	// --> ToUnicode check 'sizeof' : sizeof() returns size in bytes, NOT characters!
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!CreateRibbonBar ())
	{
		TRACE(_T("Failed to create ribbon bar\n"));
	...
	m_wndOutlookPane.AddButton(theApp.LoadIcon(IDR_RIBBONTYPE), "Open", ID_FILE_OPEN);

	pOutlookBar->AddTab(&m_wndOutlookPane, "General", 1, TRUE);
	pOutlookBar->AddTab(&m_wndOutlookPane, "Aba", 2, TRUE);



	//cDialogTesteOutlookbar.Create(ID_VIEW_OUTLOOKBAR, this);

	//pOutlookBar->AddTab(&cDialogTesteOutlookbar, "222", 2, TRUE);
...
void CMainFrame::OnAppLook(UINT id)
{
	...
	CTabbedPane::m_StyleTabWnd = CMFCTabCtrl::STYLE_FLAT;
	...
	case ID_VIEW_APPLOOK_DEVIANT:	// <-- Added to support deviant themes!
		...
		case ID_VIEW_APPLOOK_DEVIANT:		// Added to support deviant themes!
			...
			if (pVisMan->GetRuntimeClass() == RUNTIME_CLASS(CMFCVisualManagerDeviant))
			{
				...
				pVisMan->SetClrRibbonBarBackground(SystemColors::DarkSecundary);
				pVisMan->SetClrRibbonBarTextPB(SystemColors::WHITE);
				pVisMan->SetClrRibbonBarTextHighlightedPB(SystemColors::WHITE);

				// Ribbon Category Properties
				pVisMan->SetClrRibbonCategoryBkTop(SystemColors::DarkPrimary);
				pVisMan->SetClrRibbonCategoryBkBottom(SystemColors::DarkPrimary);

				// Ribbon Panel Properties
				pVisMan->SetClrRibbonPanelBkTop(SystemColors::DarkPrimary);
				pVisMan->SetClrRibbonPanelBkBottom(SystemColors::DarkPrimary);
				pVisMan->SetClrRibbonPanelCaptionBk(SystemColors::DarkPrimary);
				pVisMan->SetClrRibbonPanelCaptionTextPB(SystemColors::DarkSecundary);
				pVisMan->SetClrRibbonPanelCaptionTextHighlightedPB(SystemColors::DarkComplementary_2);
				pVisMan->SetClrRibbonPanelTextPB(SystemColors::WHITE);
				pVisMan->SetClrRibbonPanelTextHighlightedPB(SystemColors::WHITE);

				// Ribbon Panel outline colour
				pVisMan->SetClrRibbonPanelOutline(SystemColors::DarkPrimary);

				// Dockable Pane Caption					
				pVisMan->SetClrDockablePaneCaption(SystemColors::DarkSecundary);
				pVisMan->SetClrDockablePaneCaptionHighlighted(SystemColors::DarkSecundary);
				pVisMan->SetClrDockablePaneCaptionText(SystemColors::LightSecundary);
				pVisMan->SetClrDockablePaneCaptionTextHighlighted(SystemColors::WHITE);

				// Menu Text				
				pVisMan->SetClrMenuText(SystemColors::WHITE);
				pVisMan->SetClrMenuTextHighlighted(SystemColors::WHITE);
				pVisMan->SetClrMenuTextDisabled(SystemColors::DarkComplementary_2);
				pVisMan->SetClrMenuItemBackgroundHighlight(SystemColors::DarkPrimary);

				// Outlook Bar 					
				pVisMan->SetClrOutlookPageButtonText(SystemColors::DarkPrimary);
				pVisMan->SetClrOutlookPageButtonTextHighlighted(SystemColors::BLACK);
				pVisMan->SetClrOutlookPageButtonTextSelected(SystemColors::DarkPrimary);
				pVisMan->SetClrOutlookPageButtonBackground(SystemColors::LightPrimary);
				pVisMan->SetClrOutlookPageButtonBackgroundHighlighted(SystemColors::LightSecundary);
				pVisMan->SetClrOutlookPageButtonBackgroundSelected(SystemColors::LightComplementary_1);
				pVisMan->SetClrOutlookCaptionText(SystemColors::DarkSecundary);
				pVisMan->SetClrOutlookCaptionBackground(SystemColors::LightComplementary_1);
				pVisMan->SetClrOutlookBarSplitter(SystemColors::DarkSecundary);

				//tab (CMFCTabCtrl)				
				pVisMan->SetClrTabText(SystemColors::DarkPrimary);
				pVisMan->SetClrTabHighlighted(SystemColors::BLACK);
				pVisMan->SetClrTabTextSelected(SystemColors::DarkPrimary);
				pVisMan->SetClrTabBackground(SystemColors::LightComplementary_1);
				pVisMan->SetClrTabBackgroundHighlighted(SystemColors::LightSecundary);
				pVisMan->SetClrTabBackgroundSelected(SystemColors::WHITE);
				pVisMan->SetClrTabsAreaBackground(SystemColors::DarkSecundary);
...
void CMainFrame::Add_Category7()
{
	CMFCRibbonCategory* pCategory = m_wndRibbonBar.AddCategory(_T("&Deviant Editor"), IDB_RIBBON_CATEGORY3_SMALL, IDB_RIBBON_CATEGORY7_LARGE);

	// Category panel
	CMFCRibbonPanel* pPanel1 = pCategory->AddPanel(_T("Category"));

	CMFCRibbonColorButton* pBtn1 = new CMFCRibbonColorButton(ID_RIBBON_CBTN_GRADSTART, _T("Gradient start"), TRUE, 1, 0);
	pBtn1->SetAlwaysLargeImage();
	pBtn1->SetDefaultCommand(FALSE);
	pBtn1->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn1->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	pPanel1->Add(pBtn1);

	CMFCRibbonColorButton* pBtn2 = new CMFCRibbonColorButton(ID_RIBBON_CBTN_GRADEND, _T("Gradient end"), TRUE, 1, 1);
	pBtn2->SetAlwaysLargeImage();
	pBtn2->SetDefaultCommand(FALSE);
	pBtn2->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn2->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	...
	// Panel panel
	CMFCRibbonPanel* pPanel2 = pCategory->AddPanel(_T("Panel"));

	CMFCRibbonColorButton* pBtn3 = new CMFCRibbonColorButton(ID_RIBBONPANEL_CBTN_GRADSTART, _T("Gradient start"), TRUE, 1, 2);
	pBtn3->SetAlwaysLargeImage();
	pBtn3->SetDefaultCommand(FALSE);
	pBtn3->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn3->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	...
	CMFCRibbonColorButton* pBtn4 = new CMFCRibbonColorButton(ID_RIBBONPANEL_CBTN_GRADEND, _T("Gradient end"), TRUE, 1, 3);
	pBtn4->SetAlwaysLargeImage();
	pBtn4->SetDefaultCommand(FALSE);
	pBtn4->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn4->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	pPanel2->Add(pBtn4);

	CMFCRibbonColorButton* pBtn9 = new CMFCRibbonColorButton(ID_RIBBONPANEL_CBTN_BUTTONTEXT, _T("Panel Button Text Colour"), TRUE, 1, 4);
	pBtn9->SetAlwaysLargeImage();
	pBtn9->SetDefaultCommand(FALSE);
	pBtn9->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn9->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	pPanel2->Add(pBtn9);

	CMFCRibbonColorButton* pBtn9_2 = new CMFCRibbonColorButton(IDRIBBONPANELCAPTIONBK_CBTN, _T("Caption Back Colour"), TRUE, 1, 12);
	pBtn9_2->SetAlwaysLargeImage();
	pBtn9_2->SetDefaultCommand(FALSE);
	pBtn9_2->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn9_2->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	pPanel2->Add(pBtn9_2);

	CMFCRibbonColorButton* pBtn5 = new CMFCRibbonColorButton(ID_RIBBONPANEL_CBTN_TEXT, _T("Caption Text Colour"), TRUE, 1, 5);
	pBtn5->SetAlwaysLargeImage();
	pBtn5->SetDefaultCommand(FALSE);
	pBtn5->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn5->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	pPanel2->Add(pBtn5);

	CMFCRibbonColorButton* pBtn6 = new CMFCRibbonColorButton(ID_RIBBONPANEL_CBTN_TEXTHILITE, _T("Caption Text Hilight Colour"), TRUE, 1, 6);
	pBtn6->SetAlwaysLargeImage();
	pBtn6->SetDefaultCommand(FALSE);
	pBtn6->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn6->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	...
	CMFCRibbonColorButton* pBtn7 = new CMFCRibbonColorButton(ID_RIBBONPANEL_CBTN_OUTLINE, _T("Box Outline Colour"), TRUE, 1, 7);
	pBtn7->SetAlwaysLargeImage();
	pBtn7->SetDefaultCommand(FALSE);
	pBtn7->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn7->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	pPanel2->Add(pBtn7);

	// Menu bar panel
	CMFCRibbonPanel* pPanel3 = pCategory->AddPanel(_T("Menu Bar"));

	CMFCRibbonColorButton* pBtn8 = new CMFCRibbonColorButton(ID_RIBBONBARBK_CBTN, _T("Background"), TRUE, 1, 8);
	pBtn8->SetAlwaysLargeImage();
	pBtn8->SetDefaultCommand(FALSE);
	pBtn8->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn8->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	pPanel3->Add(pBtn8);

	CMFCRibbonColorButton* pBtn10 = new CMFCRibbonColorButton(ID_RIBBONBARTXT_CBTN, _T("Text"), TRUE, 1, 9);
	pBtn10->SetAlwaysLargeImage();
	pBtn10->SetDefaultCommand(FALSE);
	pBtn10->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn10->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	pPanel3->Add(pBtn10);

	CMFCRibbonColorButton* pBtn11 = new CMFCRibbonColorButton(ID_RIBBONBARTXTHILITE_CBTN, _T("Text"), TRUE, 1, 10);
	pBtn11->SetAlwaysLargeImage();
	pBtn11->SetDefaultCommand(FALSE);
	pBtn11->EnableAutomaticButton(_T("&Automatic"), RGB(0, 0, 0));
	pBtn11->EnableOtherButton(_T("&More Colors..."), _T("More Colors"));
	pPanel3->Add(pBtn11);

	// Tools panel
	CMFCRibbonPanel* pPanel4 = pCategory->AddPanel(_T("Tools"));
	CMFCRibbonButton* pBtn12 = new CMFCRibbonButton(ID_RIBBONBARTOOLS_CBTN, _T("Get Code"), -1, 11, TRUE);
	pPanel4->Add(pBtn12);
}
...
////////////////////////////////////////////////////////////////////////////////
// Fns added by Pete to allow theme design
////////////////////////////////////////////////////////////////////////////////


CMFCVisualManager* CMainFrame::IsDeviantVisualManager()
{
	CMFCVisualManagerDeviant* pVisMan = (CMFCVisualManagerDeviant*)CMFCVisualManager::GetInstance();
	if (pVisMan->GetRuntimeClass() != RUNTIME_CLASS(CMFCVisualManagerDeviant))
	{
		AfxMessageBox(_T("You must switch to the 'Deviant' theme on the themes menu first!"));
		return NULL;
	}
	return pVisMan;
}
...
CString CMainFrame::ConvertToTextRGB(COLORREF clr)
{
	CString strRetVal;
	strRetVal.Format(_T("RGB(%d, %d, %d)"), GetRValue(clr), GetGValue(clr), GetBValue(clr));
	return strRetVal;
}

void CMainFrame::OnBtnRibbonCategoryGradStart()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan=(CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(ID_RIBBON_CBTN_GRADSTART);

		pVisMan->SetClrRibbonCategoryBkTop(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonCategoryGradEnd()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(ID_RIBBON_CBTN_GRADEND);

		pVisMan->SetClrRibbonCategoryBkBottom(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonPanelCaptionBk()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(IDRIBBONPANELCAPTIONBK_CBTN);

		pVisMan->SetClrRibbonPanelCaptionBk(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonPanelGradStart()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(ID_RIBBONPANEL_CBTN_GRADSTART);

		pVisMan->SetClrRibbonPanelBkTop(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonPanelGradEnd()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(ID_RIBBONPANEL_CBTN_GRADEND);

		pVisMan->SetClrRibbonPanelBkBottom(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonPanelCaptionText()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(ID_RIBBONPANEL_CBTN_TEXT);
		pVisMan->SetClrRibbonPanelCaptionTextPB(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonPanelCaptopnTextHilite()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(ID_RIBBONPANEL_CBTN_TEXTHILITE);
		pVisMan->SetClrRibbonPanelCaptionTextHighlightedPB(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonPanelButtonText()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(ID_RIBBONPANEL_CBTN_BUTTONTEXT);
		pVisMan->SetClrRibbonPanelTextPB(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonBarBackground()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(ID_RIBBONBARBK_CBTN);
		pVisMan->SetClrRibbonBarBackground(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonPanelOutline()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(ID_RIBBONPANEL_CBTN_OUTLINE);
		pVisMan->SetClrRibbonPanelOutline(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonBarText()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(ID_RIBBONBARTXT_CBTN);
		pVisMan->SetClrRibbonBarTextPB(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonBarTextHilite()
{
	// Only party on deviant class!
	CMFCVisualManagerDeviant* pVisMan;
	if ((pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager()) != NULL)
	{
		// Get pointer to colour button
		CMainFrame* pMF = (CMainFrame*)AfxGetMainWnd();
		CMFCRibbonColorButton* pBtn = (CMFCRibbonColorButton*)pMF->GetRibbonBar()->FindByID(ID_RIBBONBARTXTHILITE_CBTN);
		pVisMan->SetClrRibbonBarTextHighlightedPB(pBtn->GetColor());
		pMF->RedrawWindow(NULL, NULL, RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW | RDW_FRAME | RDW_ERASE);
	}
}

void CMainFrame::OnBtnRibbonBarGetCode()
{
	CMFCVisualManagerDeviant*pVisMan = (CMFCVisualManagerDeviant*)IsDeviantVisualManager();

	CCodeDlg coder;
	coder.m_strCode.Format(
		_T("// Ribbon Bar properties\r\n")
		_T("pVisMan->SetClrRibbonBarBackground(%s);\r\n")
		_T("pVisMan->SetClrRibbonBarTextPB(%s);\r\n")
		_T("pVisMan->SetClrRibbonBarTextHighlightedPB(%s);\r\n")
		_T("\r\n")
		_T("// Ribbon Category Properties\r\n")
		_T("pVisMan->SetClrRibbonCategoryBkTop(%s);\r\n")
		_T("pVisMan->SetClrRibbonCategoryBkBottom(%s);\r\n")
		_T("\r\n")
		_T("// Ribbon Panel Properties\r\n")
		_T("pVisMan->SetClrRibbonPanelBkTop(%s);\r\n")
		_T("pVisMan->SetClrRibbonPanelBkBottom(%s);\r\n")
		_T("pVisMan->SetClrRibbonPanelCaptionBk(%s);\r\n")
		_T("pVisMan->SetClrRibbonPanelCaptionTextPB(%s);\r\n")
		_T("pVisMan->SetClrRibbonPanelCaptionTextHighlightedPB(%s);\r\n")
		_T("pVisMan->SetClrRibbonPanelTextPB(%s);\r\n")
		_T("\r\n")
		_T("// Ribbon Panel outline colour\r\n")
		_T("pVisMan->SetClrRibbonPanelOutline(%s);\r\n")
		, ConvertToTextRGB(pVisMan->GetClrRibbonBarBackground())
		, ConvertToTextRGB(pVisMan->GetClrRibbonBarTextPB())
		, ConvertToTextRGB(pVisMan->GetClrRibbonBarTextHighlightedPB())
		, ConvertToTextRGB(pVisMan->GetClrRibbonCategoryBkTop())
		, ConvertToTextRGB(pVisMan->GetClrRibbonCategoryBkBottom())
		, ConvertToTextRGB(pVisMan->GetClrRibbonPanelBkTop())
		, ConvertToTextRGB(pVisMan->GetClrRibbonPanelBkBottom())
		, ConvertToTextRGB(pVisMan->GetClrRibbonPanelCaptionBk())
		, ConvertToTextRGB(pVisMan->GetClrRibbonPanelCaptionTextPB())
		, ConvertToTextRGB(pVisMan->GetClrRibbonPanelCaptionTextHighlightedPB())
		, ConvertToTextRGB(pVisMan->GetClrRibbonPanelTextPB())
		, ConvertToTextRGB(pVisMan->GetClrRibbonPanelOutline())
	);
	coder.DoModal();
}
