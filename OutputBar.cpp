...
int COutputBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	// Create tabs window:
	if (!m_wndTabs.Create (CMFCTabCtrl::STYLE_3D, CRect (0, 0, 0, 0), this, 1))
	{
		TRACE(_T("Failed to create output tab window\n"));
		return -1;      // fail to create
	}
	m_wndTabs.SetTabBorderSize(0, 1);
	...
	if (nID != 0)
	{
		...
		else
		{
			...
			else
			{
				...
				LPCCH lpszResourceText = (LPCCH)LockResource(hGlobal);
#ifdef UNICODE
				MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, lpszResourceText, dwSize, lpszText, dwSize);	// --> ToUnicode check 'MultiByteToWideChar' : check conversion still required.
#else
...
void ErrorReport (LPCTSTR lpszPathName, CException* e)
{
	...
	{
		TRACE(traceAppMsg, 0, _T("Reporting file I/O exception on save with lOsError = $%lX.\n"),
...
void COutputBar::OnFileSaveAs() 
{
					...
					CFile::modeWrite | CFile::shareExclusive, &fe))
	{
		...
		{
			CWaitCursor wait;
			file.Write((LPCTSTR)strText, strText.GetLength () * sizeof(TCHAR));	// --> ToUnicode check 'sizeof' : sizeof() returns size in bytes, NOT characters!
			file.Close ();
		}
		CATCH_ALL(e)
		{
...
void COutputBar::OnFilePrint() 
{
    ...
    DOCINFO di;
    ::ZeroMemory (&di, sizeof (DOCINFO));	// --> ToUnicode check 'sizeof' : sizeof() returns size in bytes, NOT characters!
    di.cbSize = sizeof (DOCINFO);	// --> ToUnicode check 'sizeof' : sizeof() returns size in bytes, NOT characters!
